...
Feature: Local test
    ...
    Scenario: Execute KAM bootstrap command with default and --push-to-git=true flags
        When executing "kam bootstrap --service-repo-url $SERVICE_REPO_URL --gitops-repo-url $GITOPS_REPO_URL --git-host-access-token $GITHUB_TOKEN --private-repo-driver $PRIVATE_REPO_DRIVER --push-to-git=true" succeeds
        Then stderr should be empty

    Scenario: Execute KAM bootstrap command with default flags
        When executing "kam bootstrap --service-repo-url $SERVICE_REPO_URL --gitops-repo-url $GITOPS_REPO_URL --git-host-access-token $GITHUB_TOKEN --private-repo-driver $PRIVATE_REPO_DRIVER --overwrite" succeeds
        Then stderr should be empty
